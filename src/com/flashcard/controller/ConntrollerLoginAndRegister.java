...
public class ConntrollerLoginAndRegister extends javax.swing.JFrame {
    ...
    private void login() {
        ...
        try {
            ...
            if (user != null) {
            	ConntrollerLoginAndRegister loginAndRegister = this;
                this.dispose();
                System.out.println("User role in login method: " + user.getRole());

                if("admin".equalsIgnoreCase(user.getRole())) {
                	 new ControllerAdmin(user,loginAndRegister).setVisible(true);
                }else {
                	new ControllerUser(user,loginAndRegister).setVisible(true);
                }
            ...
            // Xóa dữ liệu trong trường password sau khi đăng nhập hoặc gặp lỗi
    ...
    private void showMessage(Message.MessageType messageType, String message) {
        ...
        ms.showMessage(messageType,  "Đăng nhập thất bại",message, 3000);
        TimingTarget target = new TimingTargetAdapter() {
            @Override
            public void begin() {
                if (!ms.isShow()) {
